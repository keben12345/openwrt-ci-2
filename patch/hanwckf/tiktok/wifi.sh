...
wifi_ssid=$(cat /sys/class/net/br-lan/address 2>/dev/null | awk -F: '{print $5 $6}' | tr 'a-z' 'A-Z')-WiFi
wifi_num=15
...
for i in $(seq 1 $wifi_num); do
    ...
    uci set wireless.wifinet${wifinet_num}.ssid="${wifi_ssid}-${i}"
    uci set wireless.wifinet${wifinet_num}.encryption='psk2+ccmp'
    uci set wireless.wifinet${wifinet_num}.key="$wifi_password"
    ...
    uci set network.wifi${i}.ipaddr="10.10.${i}.1"
